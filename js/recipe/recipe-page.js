...
async function getPageList(page, searchKey, searchValue) {
  ...
  if (searchKey && searchValue) {
    url = `https://www.woohyo.store/api/recipes/paging/search?page=${page}&size=${pageSize}&${searchKey}=${searchValue}`;
  } else {
    url = `https://www.woohyo.store/api/recipes/paging?page=${page}&size=${pageSize}`;
  }
    ...
    list.addEventListener("click", async (e) => {
      if (e.target.tagName === "BUTTON") {
        ...
        const response = await fetch(
          `https://www.woohyo.store/api/recipes?no=${no}`
        );
        const result = await response.json();
        ...
        removeDataBtn.addEventListener("click", async () => {
          ...
          const responce = await fetch(
            `https://www.woohyo.store/api/recipes/${no}`,
            {
              method: "DELETE",
              headers: {
